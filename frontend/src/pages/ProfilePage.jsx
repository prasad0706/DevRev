...
import { Award, CheckCircle2, MessageSquare, Flame } from 'lucide-react';

export default function ProfilePage({ username = 'alex_dev', onNavigate }) {
  const profile = {
    username: username,
    role: 'Verified Code Reviewer',
    karma: 620,
    acceptedCount: 15,
    joined: 'Aug 2026',
    bio: 'Frontend Performance & React Internals Enthusiast.'
  };
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between text-xs text-[#8A8A8E] mt-2 pt-1 border-t border-[#242427]">
                  ...
                  <div className="flex gap-2">
                    {post.tags.map(t => (
                      <span key={t} className="text-[#8A8A8E]">#{t}</span>
                    ))}
                  </div>
                </div>
              </div>
